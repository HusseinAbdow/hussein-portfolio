...
export default function IntroLoader() {
  ...
  useEffect(() => {
    // If reduced motion is requested, slide out after 500ms; otherwise keep the intro under two seconds.
    const displayDuration = shouldReduceMotion ? 500 : 1200;
    ...
    <AnimatePresence>
      {isVisible && (
        <motion.div
          ...
          animate={{ y: 0 }}
          exit={{ y: "-100%" }}
          transition={{ duration: shouldReduceMotion ? 0.6 : 0.35, ease: [0.62, 0.05, 0.01, 0.99] }} // Custom smooth easeInOut panel slide
